...
model_1 <- glm(hosp ~ age, data = hospital, family = 'binomial')
summary(model_1)
...
model_2 <-  glm(hosp ~ age + I(age^2), data=hospital, family = 'binomial')
summary(model_2)
confint(model_2)
...
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -work_norm)
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -inc_hh)
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -inc_tot)
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -age)
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -I(age^2))
lrtest(model_3,model_r)
model_r <- update(model_3, . ~ . -I(age^2) - age)
lrtest(model_3,model_r)


png('outputs/box_plot.png')
ggplot(hospital, aes(x = work_norm, y = age)) +
  geom_boxplot() 
dev.off()
library(MASS)
model_4 <- step(nullmod, scope = list(lower = nullmod, upper = model_3), direction = "both", criterion = 'AIC')
AIC(model_4)
BIC(model_4)
PseudoR2(model_4, which = "McFadden")
lrtest(model_4, model_3)

summary(model_4)
confint(model_4)
xtable(summary(model_4))
exp(model_4$coefficients)
exp(confint(model_4))
hospital$health<- relevel(hospital$health, "good")
model_3 <- glm(hosp ~ age + I(age^2) + health + sex + civilst + exercise + work_norm + inc_hh + inc_tot, data = hospital)
model_5 <- step(nullmod, scope = list(lower = nullmod, upper = model_3), direction = "both", criterion = 'BIC', k = log(nrow(hospital)))
summary(model_5)
confint(model_5)
...
AIC(model_5)
BIC(model_5)
PseudoR2(model_5, which = "McFadden")
lrtest(model_5, model_3)
lrtest(model_5, model_4)

pd <- data.frame(predict(model_5, newdata = hospital, type = "response", se.fit=TRUE))
pd <- transform(pd, Fitted = ilink(fit), Upper = ilink(fit + (2 * se.fit)),
                Lower = ilink(fit - (2 * se.fit)))
temp = cbind(hospital, pd)
png('outputs/preds_bic.png', width=750, height=400)
options(repr.plot.width=20, repr.plot.height=8)
ggplot(temp, aes(x= age, fill = health)) + 
  geom_line(aes(y = fit), size = 1) +
  geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.5, size = 0.2) +
  xlab("Age") + ylab("Hosp") +
  facet_wrap(~ sex)
dev.off()
hospital$health<- relevel(hospital$health, "bad")
model_3 <- glm(hosp ~ age + I(age^2) + health + sex + civilst + exercise + work_norm + inc_hh + inc_tot, data = hospital)
model_5 <- step(nullmod, scope = list(lower = nullmod, upper = model_3), direction = "both", criterion = 'BIC', k = log(nrow(hospital)))
options(scipen=999)
summary(model_5)
...
hospital$new_health <- factor(hospital$health,
                          levels = c(1, 2, 3),
                          labels = c("Good", "Bad/Somewhere in between", "Bad/Somewhere in between"))
hospital$new_health<- relevel(hospital$new_health, "Good")
model_3 <- glm(hosp ~ age + I(age^2) + new_health + sex + civilst + exercise + work_norm + inc_hh + inc_tot, data = hospital)
model_5 <- step(nullmod, scope = list(lower = nullmod, upper = model_3), direction = "both", criterion = 'BIC', k = log(nrow(hospital)))
summary(model_5)
confint(model_5)
exp(model_5$coefficients)
