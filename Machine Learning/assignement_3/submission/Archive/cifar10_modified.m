function cifar10_starter()
    ...
    %Baseline layers
    net.layers = {};
    net.layers{end+1} = struct('type', 'input', ... %1
        'params', struct('size', [32, 32, 3]));
    
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,3,16)/sqrt(5*5*3/2), 'biases', zeros(16,1)),... %2
        'padding', [2 2]);
    
    net.layers{end+1} = struct('type', 'relu'); %3


    net.layers{end+1} = struct('type', 'maxpooling'); %4
    
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,16,24)/sqrt(5*5*16/2), 'biases', zeros(24,1)),... %5
        'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,24,24)/sqrt(5*5*24/2), 'biases', zeros(24,1)),... %5
        'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'maxpooling'); %7
    
    net.layers{end+1} = struct('type', 'convolution',... %8
        'params', struct('weights', 0.1*randn(3,3,24,16)/sqrt(3*3*24/2), 'biases', zeros(16,1)),...
        'padding', [1 1]);
    ...
    training_opts = struct('learning_rate', 1e-2,...
        'iterations', 6000,...
        'batch_size', 32,...
        'momentum', 0.95,...
        'weight_decay', 0.004);
