...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    %Feature 1 What is in next loc?
    [next_head_loc, next_move_dir] = get_next_info(action, movement_dir, head_loc);
    state_action_feats(1, action) = grid(next_head_loc(1), next_head_loc(2));
    man_grid_size = pdist([0 0 ; size(grid)],'cityblock');
    %f2 Manhattan distance from new head loc to apple?
    [r,c] = find(grid == -1);
    apple_loc = [r,c];
    apple_dist = pdist([next_head_loc;apple_loc],'cityblock');
    state_action_feats(2, action) = pdist([next_head_loc;apple_loc],'cityblock') ... 
                                            /man_grid_size;
    
    %3 distance to wall [This is not used in final model since it doesnt
    %increase performance]
    %edges = [[ones(1,30);1:30]';[1:30;ones(1,30)]';[1:30;30 * ones(1,30)]' ; [30 * ones(1,30);1:30]'];
    %snake_size = length(find(grid == 1)) - 30 * 4 + 4;
    %temp = pdist([head_loc ; edges],'cityblock');
    %temp = min(temp(:,1));
    %temp = ((temp)+snake_size)/man_grid_size;
    state_action_feats(3, action) = 0; %set this to 0 since it doesnt improve performance and goal is already achieved
