...
public class SudokuViewer {
	...
	private static Font font1 = new Font("SansSerif", Font.PLAIN, 20);
	...
	public SudokuViewer(Sudoku sudoku) {
		...
		this.sudoku = sudoku;
		SwingUtilities.invokeLater(() -> createWindow("Sudoku", 700, 700));
	}

	private void createWindow(String title, int width, int height) {
		JFrame frame = new JFrame(title);
		grid = new JTextField[sudoku.getDimension()][sudoku.getDimension()];
		...
		solve.addActionListener(e -> {
			if (!updateSudoku()) {
				optionPane.showMessageDialog(null, "Please remove invalid inputs, please make sure all inputs are in the range [1,9]");
			} else {
				sudoku.solve();
				updateGrid();
				if (sudoku.isAllValid()) {
					optionPane.showMessageDialog(null, "Solved Sudoku");
				} else {
					optionPane.showMessageDialog(null, "Soduku is unsolvable");
				}
			}
