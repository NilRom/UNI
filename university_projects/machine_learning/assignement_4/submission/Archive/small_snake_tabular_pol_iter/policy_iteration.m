...
while 1
    ...
    while 1
        ...
        for state_idx = 1 : nbr_states
            ...
            switch next_state_idx
                case -1
                values(state_idx) = rewards.apple; %Found apple
                case 0
                values(state_idx) = rewards.death; %Died
                otherwise
                values(state_idx) = rewards.default + gamm * values(next_state_idx); %Nothing happened
            end
    ...
    for state_idx = 1 : nbr_states
        ...
        possible_states = next_state_idxs(state_idx,:);
        temp = zeros(3,1);
        
        for i = 1:3 
            next_idx = possible_states(i);
            switch next_idx
                case -1
                temp(i) = rewards.apple; %Found apple
                case 0
                temp(i) = rewards.death; %Died
                otherwise
                temp(i) = rewards.default + gamm * values(next_idx); %Nothing happened
            end
        end
        [~, best_action] = max(temp);
