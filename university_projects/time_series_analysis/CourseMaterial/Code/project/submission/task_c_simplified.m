...
y_s = filter(deseasoner, 1, y);
u = df.tp_stu;
u_s = filter(deseasoner, 1, u);
%[ model_init_b, ey, acfEst, pacfEst ] = estimateBJ( y_s, u_s, [1 1], [1 1 1], [1], [1 1], 'simpler', 48 );
model_init_b = foundModel;
...
%Purge some parameters
K_c([ 3 4 5 24 25 26 27 28 29 ]) = 0;
K_b([ 2 3 4 24 25 26]) = 0;
K_a([5 6 7 27 28 29]) = 0;
...
xut_t1 = [K_b(idx_b+1)]';
og_params = [K_a(idx_a+1) K_c(idx_c+1) K_b(idx_b+1)]';
Rxx_1 = 1e-1 * eye(length(xt_t1));
Ruu_1 = 1e-1 * eye(length(xut_t1));
Re =  1e-6 * eye(length(xt_t1)); %make smaller
Reu =  1e-6 * eye(length(xut_t1)); %make smaller
Rw = 1; %look at variance of previous residuals
...
saveas(gcf,"Code/project/figs/simple_kalman_param_estimation.png")
